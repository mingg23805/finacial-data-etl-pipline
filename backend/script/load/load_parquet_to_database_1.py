...
def upsert_parquet_to_postgres( file_path,table_name,columns,conflict_cols,db_params: dict):
    ...
    cur.execute(f"CREATE TEMP TABLE {staging_table} AS SELECT * FROM {table_name} WHERE 1=0;")

    # 3. COPY data vào staging
    buffer = io.StringIO()
    df.to_csv(buffer, index=False, header=False)
    buffer.seek(0)
    cur.copy_from(buffer, staging_table, sep=",")

    # 4. MERGE vào bảng chính
...
def load_companies_to_db():
    ...
    upsert_parquet_to_postgres( last_parquet_file,"companies"
                            ...
                            "company_currency",
                            "company_location"],
                            conflict_cols=['company_ticket', 'company_is_delisted'],
                            db_params=db_params)
...
def load_sic_to_db():
    ...
    upsert_parquet_to_postgres( last_parquet_file,"sicindustries"
                            ...
                            conflict_cols=['sic_industry', 'sic_sector'],
                            db_params=db_params)
...
def load_exchange_to_db():
    last_parquet_file= get_latest_file_in_directory("backend/data/processed/transformed_to_db_ecxchanges",".parquet")
    db_params= {
        "dbname":Config.DB_NAME,
        "user":Config.DB_USER,
        "password":Config.DB_PASSWORD,
