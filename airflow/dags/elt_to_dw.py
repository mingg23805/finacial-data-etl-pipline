...
sys.path.append('/opt/airflow/scripts/transform')
...
with DAG(
    ...
    crawl_news = BashOperator(
        task_id='crawl_news',
        bash_command='/opt/airflow/scripts/extract/crawl_news.py',
    )

    crawl_ohlcs = BashOperator(
        task_id='crawl_ohlcs',
        bash_command='/opt/airflow/scripts/extract/crawl_ohlcs.py',
    )

    load_api_to_parquet = BashOperator(
        task_id='load_api_to_parquet',
        bash_command='/opt/airflow/scripts/load/load_api_to_parquet.py',
    )

    load_db_to_parquet = BashOperator(
        task_id='load_db_to_parquet',
        bash_command='/opt/airflow/scripts/load/load_db_to_parquet.py',
    )

    def run_shell_script():
        result = subprocess.run(['bash', '/opt/airflow/scripts/load/load_parquet_to_hdfs.sh'], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
